...
public class InteractiveQueries {
    ...
    public List<PipelineMetadata> getMetaData() {
        return streams.allMetadataForStore(TopologyProducer.PLAYER_MATCHES_STORE)
                .stream()
    ...
    public QueryResult getPlayerMatchesStore(String id) {
        StreamsMetadata metadata = streams.metadataForKey(
                TopologyProducer.PLAYER_MATCHES_STORE,
                id,
    ...
    private ReadOnlyKeyValueStore<String, String> getPlayerMatchesStore () {
        while (true) {
            try {
                return streams.store(TopologyProducer.PLAYER_MATCHES_STORE, QueryableStoreTypes.keyValueStore());
